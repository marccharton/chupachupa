...
namespace ChupaChupa_Server
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            User user = userdao.findById(108);

            if (user == null) {
                Console.WriteLine("User is null exiting...");
                Console.ReadKey();
                System.Environment.Exit(42);
            }

            Category cat = user.Categories.FirstOrDefault(c => c.IdEntity == 77);
            if (cat == null)
            {
                Console.WriteLine("category is null exiting...");
                Console.ReadKey();
                System.Environment.Exit(42);
            }
...
//             using (ServiceHost host = new ServiceHost(typeof(ServiceContract), baseAddress))
//             {
...
// 
//                 UserNamePasswordServiceCredential test = new UserNamePasswordServiceCredential();
//                 test.UserNamePasswordValidationMode = UserNamePasswordValidationMode.Custom;
//                 test.CustomUserNamePasswordValidator = new AuthenticationValidator();
//                 ServiceCredentials cred = new ServiceCredentials;
// //                UserNamePasswordValidationCredential toto;
// 
// 
// 
//                 host.Open();
